...
describe('src/helpers/header.ts', () => {
  ...
  describe('contentTypeJson', () => {
    ...
    test('should set Content-Type if not set and data is PlainObject', () => {
      const headers: any = {}
      contentTypeJson(headers, { a: 1 })
      expect(headers['Content-Type']).toBe('application/json; charset=utf-8')
    })

    test('should not set Content-Type if data is not PlainObject', () => {
      const headers: any = {}
      contentTypeJson(headers, new URLSearchParams('a=b'))
      expect(headers['Content-Type']).toBeUndefined()
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(contentTypeJson(undefined, {})).toBeUndefined()
      expect(contentTypeJson(null, {})).toBeNull()
    })
  })
